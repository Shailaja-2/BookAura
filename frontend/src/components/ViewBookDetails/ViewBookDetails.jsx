...
const ViewBookDetails = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const response = await axios.get(
        `https://bookaura-4.onrender.com/api/v1/get-book-by-id/${id}`
      );
  ...
  const handleFavourite = async () => {
    const response = await axios.put(
      "https://bookaura-4.onrender.com/api/v1/add-book-to-favourite",
      {},
      { headers }
    );
  ...
  const handleCart = async () => {
    const response = await axios.put(
      "https://bookaura-4.onrender.com/api/v1/add-to-cart",
      {},
      { headers }
    );
    alert(response.data.message);
  ...
  const deleteBook = async () => {
    const response = await axios.delete(
      "https://bookaura-4.onrender.com/api/v1/delete-book",
      { headers }
    );
    alert(response.data.message);
    navigate("/all-books");
